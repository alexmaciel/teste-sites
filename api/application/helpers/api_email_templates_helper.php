...
function parse_email_template_merge_fields($template, $merge_fields)
{
    ...
    if (!class_exists('api_merge_fields', false)) {
        $CI->load->library('merge_fields/api_merge_fields');
    }

    $api_merge_fields = new api_merge_fields();

    $merge_fields = array_merge($merge_fields, $api_merge_fields->get_other_merge_fields());
...
function get_email_template_language($template_slug, $email)
{
    ...
    if (total_rows(db_prefix() . 'staff', array(
            'email' => $email
    ...
    $hook_data = hooks()->apply_filters('email_template_language', $hook_data);
